...
enum { AUDIO_S8, AUDIO_U8, AUDIO_S16LSB };
...
SoundAdLibPC::SoundAdLibPC(const uint8_t *data, uint32_t size, int freq, bool bMAME) : _driver(0), _trackEntries(), _soundDataPtr(0) {
...
void SoundAdLibPC::internalLoadFile(const uint8_t *data, uint32_t size) {
  if (data == NULL || size <= 120) {
    return;
  }
